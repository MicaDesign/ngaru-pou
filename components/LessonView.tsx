...
export default function LessonView({ level, lessons, lesson }: Props) {
  const [completedWeeks, setCompletedWeeks] = useState<number[]>([]);

  useEffect(() => {
    getCompletedWeeksForLevel(level.slug)
      .then(setCompletedWeeks)
      .catch(() => {});
  ...
  function handleComplete() {
    setCompletedWeeks((prev) =>
      prev.includes(lesson.weekNumber) ? prev : [...prev, lesson.weekNumber],
    );
  }
        ...
        <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-8 xl:gap-12 px-6 md:px-10 py-10">
          <main className="flex-1 min-w-0">
            <div className="space-y-10">
              ...
              <Section
                title="key features"
                items={bullets(lesson.keyFeatures)}
              />
              <Section
                title="key vocabulary"
                items={bullets(lesson.keyVocabulary)}
              />
              ...
              {lesson.lyricsDownloadUrl && (
                <a
                  href={lesson.lyricsDownloadUrl}
                  target="_blank"
