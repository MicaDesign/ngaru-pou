...
- Their **credentials** (username + bcrypt-hashed PIN) are stored in **Upstash Redis** (provisioned through the Vercel Storage / Marketplace integration; the older `@vercel/kv` wrapper is deprecated, code uses `@upstash/redis` directly). Key: `student:<lowercase-username>`. Value shape: `{ id, parent_member_id, first_name, last_name, level, username, pin_hash }`. Why split from MemberStack: Data Tables can't be queried server-side without an authenticated member context (Admin REST API has no `/v1/data-records/query` endpoint; `client.memberstack.com` 403s for unauthenticated public-key calls), so making them work for student auth would have required a public read rule on the table — which would expose every PIN. Redis is server-only and PINs are bcrypt-hashed (cost 10) before write.
- Login flow: `POST /api/student-login` { username, pin } → `lib/studentCredentials.readCredentials(username)` (Redis GET via `@upstash/redis`) → `bcrypt.compare(pin, pin_hash)` → on match, the route writes the `np_student_session` httpOnly + secure + sameSite=strict cookie (8h, base64url-encoded JSON, no PIN) and returns `{ success, student }`. Mismatch returns 401 with the same generic message regardless of whether the username or PIN was wrong. Server-side validation only — no plaintext PIN ever leaves the server.
- The session is read by `app/student-dashboard/page.tsx` (server component) via `readStudentSessionFromCookies()` (uses `next/headers`). `middleware.ts` short-circuits at the edge using just the cookie name (`STUDENT_SESSION_COOKIE` lives in `lib/studentSessionConstants.ts` to keep `next/headers` out of the edge bundle). Sign-out is a no-JS form posting to `POST /api/student-logout`, which clears the cookie and 303s back to `/student-login`.
- Credentials are written by `POST /api/student-credentials` from the parent's `child-details` enrolment step. The route requires the `x-member-id` header (parent's MemberStack id), verifies the parent against the Admin API (`GET /members/{id}`), enforces `student.parent_member_id === callerId`, and NX-writes the Redis entry. 409 on duplicate username (child-details surfaces "username already taken" tied to the right child tab and rolls back the just-created `student_profiles` row via `deleteDataRecord`, so usernames stay unique without orphans).
- Required env vars (auto-injected by the Vercel Upstash integration; pull locally with `vercel env pull .env.local`): `KV_REST_API_URL`, `KV_REST_API_TOKEN`. The Upstash client also accepts `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN` as a fallback.
...
- ✅ Phase 4: Parent dashboard (view own children's profiles via DashboardView routing into ParentDashboardView for Parent-plan members)
